...
FROM (
    ...
    WHERE mint_sum_mv.tick = lower({tick: String})
    UNION ALL
    -- transfer from --
    ...
    WHERE transfer.id IN (SELECT id FROM approve_transfer WHERE approve_transfer.id = id) AND transfer.tick = lower({tick: String})
    UNION ALL
    -- transfer to --
